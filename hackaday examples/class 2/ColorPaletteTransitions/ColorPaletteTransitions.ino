...
CRGBPalette16 targetPalette( PartyColors_p ); // this should be set to the first palette you want to fade into!
...
void setup() {
    ...
    currentPalette = RainbowColors_p; // or any palette you’ve defined above
...
void ChangePalettePeriodically()
{
    ...
    if( lastSecond != secondHand) {
        ...
        if( secondHand == 15)  { targetPalette = RainbowStripeColors_p;   currentBlending = LINEARBLEND; }
        if( secondHand == 20)  { SetupPurpleAndGreenPalette();             currentBlending = LINEARBLEND; }
        if( secondHand == 25)  { SetupTotallyRandomPalette();              currentBlending = LINEARBLEND; }
        if( secondHand == 30)  { SetupBlackAndWhiteStripedPalette();       currentBlending = NOBLEND; }
        if( secondHand == 35)  { SetupBlackAndWhiteStripedPalette();       currentBlending = LINEARBLEND; }
        if( secondHand == 40)  { targetPalette = CloudColors_p;           currentBlending = LINEARBLEND; }
        if( secondHand == 45)  { targetPalette = PartyColors_p;           currentBlending = LINEARBLEND; }
        if( secondHand == 50)  { targetPalette = myRedWhiteBluePalette_p; currentBlending = NOBLEND;  }
        if( secondHand == 55)  { targetPalette = myRedWhiteBluePalette_p; currentBlending = LINEARBLEND; }
    }
}
